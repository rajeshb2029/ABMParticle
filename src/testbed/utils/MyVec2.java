...
//    v3 = Vec2.unitVector(v3);
    return (v3);
  }

  public static Vec2 unitVector(Vec2 v1) {
    Vec2 unit = new Vec2();
//    float norm = Math.sqrt(Math.pow(v1.x, 2.0) + Math.pow(v1.y, 2.0));
...
//    EuclidDistance = (float)FastMath.sqrt(FastMath.pow((v1.x-v2.x), 2)+FastMath.pow((v1.y-v2.y), 2));
    float distx, disty;
    distx = v1.x - v2.x;
    disty = v1.y - v2.y;
...
//    System.out.println("Dot: " + MyVec2.dotProduct(dir1, dir2) +
//            " length1: " + dir1.length() +
//            " length2: " + dir2.length() +
//            " angle: " + angle);
    return (angle);
  ...
  public static float getAngle(Vec2 v1) {
    double angle;
//    angle = (float) Math.acos(v1.x/v1.length());
//    angle = FastMath.acos(v1.x / v1.length());
//    if (v1.y > 0) {
//      return (float)angle;
//    } else {
//      return (float)(MathUtils.TWOPI - angle);
//    }
